...
##
def parse_euc2d_tsp_file(file_path: str) -> Dict[str, Dict]:
    ...
    for line in lines:
        ...
        ## Append the node
        graph[graph_id]["nodes"].append({"idx": int(idx), "x": float(x), "y": float(y)})
    ...
    for node in graph[graph_id]["nodes"]:
        for other_node in graph[graph_id]["nodes"]:
            ...
            graph[graph_id]["edges"].append(
                {"idx": edge_idx, "weight": weight, "start": node, "end": other_node}
            )
...
def main():
    ...
    for file_path in euc_2d_tsp_files:
        ...
        graph = parse_euc2d_tsp_file("tsplib/" + file_path)
